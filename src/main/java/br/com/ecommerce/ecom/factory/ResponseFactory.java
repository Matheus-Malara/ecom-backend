...
public class ResponseFactory {
    ...
    public <T> ResponseEntity<ApiResponse<T>> okResponse(T data, String message, String path) {
        return ResponseEntity.ok(buildResponse(data, message, path, HttpStatus.OK, null));
    }

    public <T> ResponseEntity<ApiResponse<T>> createdResponse(T data, String message, String path) {
        return ResponseEntity.status(HttpStatus.CREATED).body(buildResponse(data, message, path, HttpStatus.CREATED, null));
    }

    public <T> ResponseEntity<ApiResponse<T>> noContentResponse(String message, String path) {
        return ResponseEntity.status(HttpStatus.NO_CONTENT).body(buildResponse(null, message, path, HttpStatus.NO_CONTENT, null));
    }

    public <T> ResponseEntity<ApiResponse<T>> errorResponse(T data, String message, String path, HttpStatus status, ErrorCode errorCode) {
